...
regen_known_issues.pl

=head1 SYNOPSIS

 ./perl Porting/regen_known_issues.pl
...
my $MANIFEST = File::Spec->catfile($original_dir, 'MANIFEST');
my $copy_fh;
...
if (($show_all + $show_counts + $cpan_or_deltas ) > 1) {
    croak "--show_all, and --counts are mutually exclusive\n and none can be run with --cpan nor --deltas";
}
...
#}
        ...
        if ($cmd eq 'over') {
            ...
            if (defined $indents{$addr} and @{$indents{$addr}}) {
